...
void AddedColumns<true>::buildOutput()
{
    ...
    {
        auto& col = columns[i];
        auto apply_batch_insert = [&](const UInt64 & column_index, const UInt64 & block_addr, IColumn::Selector & selector) -> void
        {
            if (selector.size() == 0)
                return;
            if (block_addr == 0)
            {
                JoinCommon::addDefaultValues(*col, type_name[i].type, selector.size());
                return;
            }
            const auto & column_from_block = reinterpret_cast<const Block *>(block_addr)->getByPosition(column_index);
            if (is_join_get)
            {
                if (auto * nullable_col = typeid_cast<ColumnNullable *>(col.get());
                    nullable_col && !column_from_block.column->isNullable())
                {
                    nullable_col->insertIndicesFromNotNullable(*column_from_block.column, selector);
                    return;
                }
            }
            col->insertIndicesFrom(*column_from_block.column, selector);
        };

        UInt64 block_addr = lazy_output.blocks.size() > 0 ? lazy_output.blocks[0] : 0;
        IColumn::Selector row_selector;
        for (size_t j = 0; j < lazy_output.blocks.size(); ++j)
        {
            if (lazy_output.blocks[j] != block_addr)
            {
                apply_batch_insert(right_indexes[i], block_addr, row_selector);
                block_addr = lazy_output.blocks[j];
                row_selector.clear();
            }
            row_selector.emplace_back(lazy_output.row_nums[j]);
        }
        apply_batch_insert(right_indexes[i], block_addr, row_selector);
    }
}
...
void AddedColumns<true>::appendFromBlock(const Block & block, size_t row_num, bool)
{
    ...
    if (has_columns_to_add)
    {
        ...
        // UInt64 block_addr = reinterpret_cast<UInt64>(&block);
        // lazy_output.put(block_addr, row_num);
    }
}
template<>
