...
static void BM_PartialAgg(benchmark::State& state)
{
    const String plan
        = "{\n"
          "  \"extensions\": [{\n"
          ...
          "                      },\n"
          "                      \"baseSchema\": {\n"
          "                        \"names\": [\"l_partkey\", \"l_linenumber\",\"l_suppkey\", \"l_extendedprice\", \"l_shipdate\"],\n"
          "                        \"struct\": {\n"
          "                          \"types\": [{\n"
          "                            \"i64\": {\n"
          ...
          "                      \"localFiles\": {\n"
          "                        \"items\": [{\n"
          "                          \"uriFile\": \"file:///home/saber/dev/data/tpch/parquet/lineitem/part-00000-f83d0a59-2bff-41bc-acde-911002bf1b33-c000.snappy.parquet\",\n"
          "                          \"length\": \"762185954\",\n"
          "                          \"parquet\": {\n"
          "                          }\n"
          "                        }]\n"
          ...
          "        }\n"
          "      },\n"
          "      \"names\": [\"l_partkey#5\", \"min#20\", \"min#21\", \"sum#22\"]\n"
          "    }\n"
          "  }]\n"
          "}";
    for (auto _ : state)
    {
        auto context = DB::Context::createCopy(global_context);
        context->setSetting("max_bytes_before_external_group_by", 1000000000000);
        local_engine::SerializedPlanParser parser(context);
        auto query_plan = parser.parseJson(plan);
        local_engine::LocalExecutor executor;
        executor.execute(std::move(query_plan));
        size_t rows = 0;
        local_engine::SplitOptions options{
            .buffer_size = 65505,
            .data_file = "/tmp/test_shuffle/data.dat",
            .local_tmp_dir = "/tmp/test_shuffle",
            .map_id = 1,
            .partition_nums = 45,
            .exprs = {"l_partkey#5"},
            .compress_method = "LZ4"};
        ...
        {
            auto * block = executor.nextColumnar();
            rows += block->rows();
//            std::cerr << block->rows() <<std::endl;
...
static void BM_PartialFinal(benchmark::State& state)
{
    const String plan = "{\n"
                        "  \"extensions\": [{\n"
                        "    \"extensionFunction\": {\n"
    ...
    for (auto _ : state)
    {
        ...
        local_engine::SplitOptions options{
            .buffer_size = 65505,
            .data_file = "/tmp/test_shuffle/data2.dat",
            .local_tmp_dir = "/tmp/test_shuffle",
            .map_id = 1,
            .partition_nums = 45,
            .exprs = {"l_partkey#5"},
...
BENCHMARK(BM_PartialAgg)->Unit(benchmark::kMillisecond)->Iterations(10)->Repetitions(10);
BENCHMARK(BM_PartialFinal)->Unit(benchmark::kMillisecond)->Iterations(10)->Repetitions(10);
